...
from config import beta_real, alpha_real, k_real, numero_passos, I0, S0, R0, beta_chute, alpha_chute, k_chute, mus
...
def main():
    ...
    parametros_estimados, historico_beta, historico_alpha, historico_k, historico_erro, historico_delta, iteracoes = estimador.estimar(parametros_iniciais=[beta_chute, alpha_chute, k_chute])
    ...
    Plotador.plotar_convergencia(historico_beta, historico_alpha, historico_k, historico_delta, titulo="Gauss-Newton")

    # -----------------------------
    # 3. Levenberg-Marquardt Adaptativo
    # -----------------------------

    parametros_iniciais_lm = [beta_chute, alpha_chute, k_chute]

    estimador_lm_adaptativo = EstimadorLevenbergMarquardtAdaptativo(
        numero_passos=numero_passos,
        I0=I0,
        S0=S0,
        R0=R0,
        dados_observados=dados_observados
    )

    (
        parametros_lm_adaptativo,
        historico_beta_lm_adaptativo,
        historico_alpha_lm_adaptativo,
        historico_k_lm_adaptativo,
        historico_delta_lm_adaptativo,
        historico_mu_lm_adaptativo,
        iteracoes_lm_adaptativo
    ) = estimador_lm_adaptativo.estimar(parametros_iniciais=parametros_iniciais_lm)
    ...
    print(
        ...
        f"{'Mu inicial:':<28}{historico_mu_lm_adaptativo[0]:.6e}\n"
        f"{'Mu final:':<28}{historico_mu_lm_adaptativo[-1]:.6e}\n"
        f"{'Iterações:':<28}{iteracoes_lm_adaptativo}"
    )

    Plotador.plotar_convergencia(
        historico_beta_lm_adaptativo,
        historico_alpha_lm_adaptativo,
        historico_k_lm_adaptativo,
        historico_delta_lm_adaptativo,
        titulo="LM Adaptativo"
    )
    ...
    Plotador.plotar_delta_theta_comparacao(
        historico_delta,
        historico_delta_lm_adaptativo
    )

    print("\nTeste com diferentes valores iniciais de mu - LM")
    print("------------------------------------------------------------------------------------------------")
    print("mu inicial   iter   beta          alpha         k             beta*k        mu final      delta final")
    print("------------------------------------------------------------------------------------------------")

    historico_iteracoes_mu = []

    historico_delta_final_mu = []

    for mu in mus:

        estimador_lm = EstimadorLevenbergMarquardtAdaptativo(
            numero_passos=numero_passos,
            I0=I0,
            S0=S0,
            R0=R0,
        ...
        (
            ...
            historico_mu_lm,
            iteracoes_lm
        ) = estimador_lm.estimar(parametros_iniciais=parametros_iniciais_lm)

        beta_lm, alpha_lm, k_lm = parametros_lm

        produto_lm = beta_lm * k_lm

        mu_final = historico_mu_lm[-1]

        delta_final = historico_delta_lm[-1]

        historico_iteracoes_mu.append(iteracoes_lm)

        historico_delta_final_mu.append(delta_final)
