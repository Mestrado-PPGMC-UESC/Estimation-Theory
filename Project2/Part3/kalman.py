...
def obter_matrizes_kalman(modelo, estado_inicial):
    ...
    if hasattr(modelo, "taxa_natalidade"):
        ...
        # ========================================================
        # Caso multissensor
        # Exemplo: modelo.ruidos_observacao = [1.0, 1.5, 8.0]
        # ========================================================
        if hasattr(modelo, "ruidos_observacao") and modelo.ruidos_observacao is not None:

            quantidade_sensores = len(modelo.ruidos_observacao)

            H = np.ones((quantidade_sensores, 1))

            R = np.diag([
                ruido**2 for ruido in modelo.ruidos_observacao
            ])

        # ========================================================
        # Caso com apenas uma medição
        # ========================================================
        else:
            H = np.array([[1]])
...
def passo_correcao(H, R, observacao, x_predito, P_predito):
    ...
    Funciona tanto para:
    - uma única medição;
    - múltiplas medições empilhadas.
    """

    if np.isscalar(observacao):
        observacao = np.array([[observacao]])

    else:
        observacao = np.array(observacao)

        if observacao.ndim == 1:
            observacao = observacao.reshape(-1, 1)
