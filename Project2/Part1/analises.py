...
def analise_kalman_filtrado_multiplas_sementes(
    ...
    erro_seed = estimativas_filtradas - trajetoria_com_ruido[:len(estimativas_filtradas)]

    plt.figure(figsize=(12, 5))
    plt.plot(k, erro_seed, label="Erro filtrado")
    plt.axhline(0, linestyle="--")
    plt.xlabel("Passo de tempo (k)")
    plt.ylabel("Erro")
    plt.title("Erro da Estimativa Filtrada - seed 42")
    plt.legend()
    plt.grid(True)
    plt.show()
...
def analise_kalman_preditivo_multiplas_sementes(
    ...
    erro_seed = estimativas_preditivas - trajetoria_com_ruido[:len(estimativas_preditivas)]

    plt.figure(figsize=(12, 5))
    plt.plot(k, erro_seed, label="Erro preditivo")
    plt.axhline(0, linestyle="--")
    plt.xlabel("Passo de tempo (k)")
    plt.ylabel("Erro")
    plt.title("Erro da Estimativa Preditiva - seed 42")
    plt.legend()
    plt.grid(True)
    plt.show()
...
def analise_kalman_preditivo_corretivo_multiplas_sementes(
    ...
    plt.plot(k, trajetoria_sem_ruido[:len(k)], label="Sem ruído", linewidth=2)
    plt.plot(k, trajetoria_com_ruido[:len(k)], label="Com ruído", alpha=0.8)
    plt.plot(k, estimativas_corrigidas, label="Estimativa corrigida", linewidth=2)
    plt.xlabel("Passo de tempo (k)")
    plt.ylabel("Preço")
    ...
    erro_seed = estimativas_corrigidas - trajetoria_com_ruido[:len(estimativas_corrigidas)]

    plt.figure(figsize=(12, 5))
    plt.plot(k, erro_seed, label="Erro preditivo-corretivo")
    plt.axhline(0, linestyle="--")
    plt.xlabel("Passo de tempo (k)")
    plt.ylabel("Erro")
    plt.title("Erro da Estimativa Preditiva-Corretiva - seed 42")
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    desvio_nrmse_pred = np.std(lista_nrmse_pred)
    desvio_nrmse_corr = np.std(lista_nrmse_corr)

    print(f"Média NRMSE da predição para {numero_iteracoes} sementes: {media_nrmse_pred:.6f}")
    print(f"Desvio padrão NRMSE da predição: {desvio_nrmse_pred:.6f}")

    print(f"Média NRMSE da correção para {numero_iteracoes} sementes: {media_nrmse_corr:.6f}")
    print(f"Desvio padrão NRMSE da correção: {desvio_nrmse_corr:.6f}")

    return {
        "estimativas_preditivas": estimativas_preditivas,
        "estimativas_corrigidas": estimativas_corrigidas,
        "media_nrmse_pred": media_nrmse_pred,
        "media_nrmse_corr": media_nrmse_corr,
        "desvio_nrmse_pred": desvio_nrmse_pred,
        "desvio_nrmse_corr": desvio_nrmse_corr,
        "erro_seed": erro_seed
    }